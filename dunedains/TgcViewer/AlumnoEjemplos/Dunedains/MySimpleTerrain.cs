...
namespace AlumnoEjemplos.Dunedains
{
    ...
    public class MySimpleTerrain
    {
        ...
        public void loadHeightmap(string heightmapPath, float pscaleXZ, float pscaleY, Vector3 center)
        {
            ...
            width = (float)heightmapData.GetLength(0);
            length = (float)heightmapData.GetLength(1);
            ...
            else
            {
                for (int i = 0; i < width - 1; i++)
                {
                    ...
                    {
                        //Vertices                        
                        Vector3 v1 = new Vector3((float)Math.Sin(center.X + i * scaleXZ), (float)Math.Sin(center.Y + heightmapData[i, j] * scaleY), (float)Math.Sin(center.Z + j * scaleXZ));
                        Vector3 v2 = new Vector3((float)Math.Sin(center.X + i * scaleXZ), (float)Math.Sin(center.Y + heightmapData[i, j + 1] * scaleY), (float)Math.Sin(center.Z + (j + 1) * scaleXZ));
                        Vector3 v3 = new Vector3((float)Math.Sin(center.X + (i + 1) * scaleXZ), (float)Math.Sin(center.Y + heightmapData[i + 1, j] * scaleY), (float)Math.Sin(center.Z + j * scaleXZ));
                        Vector3 v4 = new Vector3((float)Math.Sin(center.X + (i + 1) * scaleXZ), (float)Math.Sin(center.Y + heightmapData[i + 1, j + 1] * scaleY), (float)Math.Sin(center.Z + (j + 1) * scaleXZ));
        ...
        public void render()
        {
            ...
            //loadMovimientoOceano(Parametros.getCurrentHMap(), Parametros.getCurrentScaleXZ(), (float)Math.Sin(Parametros.getCurrentScaleY()*GuiController.Instance.ElapsedTime), new Vector3(0, 0, 0));

            d3dDevice.SetStreamSource(0, vbTerrain, 0);
            d3dDevice.DrawPrimitives(PrimitiveType.TriangleList, 0, totalVertices / 3);
        }

        public void loadMovimientoOceano(string heightmapPath, float pscaleXZ, float pscaleY, Vector3 center)
        {
            scaleXZ = pscaleXZ;
            scaleY = pscaleY;
            ...
            float dj = length * kj;
            float alto1 = 2;
            float alto2 = (float)(alto1 + Math.Sin(GuiController.Instance.ElapsedTime));
            float alto3 = (float)(alto2 + Math.Sin(GuiController.Instance.ElapsedTime));
        ...
        public void executeRender(Effect effect)
        {
            ...
            //Render terrain 
            effect.SetValue("aux_Tex", terrainTexture);
