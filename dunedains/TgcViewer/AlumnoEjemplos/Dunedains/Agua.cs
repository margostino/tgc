...
namespace AlumnoEjemplos.Dunedains
{
    public class Agua
    {
        public const int DIMENSION = 150;
        ...
        public const int HEIGHT_CENTER = 300;
        ...
        private VerticeAgua[,] crearMovimiento(float tiempo)
        {
            ...
            if (efecto == "velocidad var")
            {
                ...
                for (int z = 0; z < TOTAL_VERTICES_DIMENSION; z++)
                {
                    for (int x = 0; x < TOTAL_VERTICES_DIMENSION; x++)
                    {
                        vertices[x, z].setY(HEIGHT_CENTER + (amplitud * (float)Math.Sin(tiempo * D + vertices[x, z].Pos.X * frecuencia) * (float)Math.Cos(tiempo * D + vertices[x, z].Pos.Z * frecuencia)));
                    }
                }
            }
            return vertices;
